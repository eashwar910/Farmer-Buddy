...
export async function summarizeKeyframes(keyframes, genAI) {
  ...
  for (const modelName of MODELS_TO_TRY) {
    try {
      ...
      console.log(`  ✅ Gemini response received via ${modelName} (${text.length} characters)`);
  ...
  if (!text) {
    ...
    // Check for specific error types passing through the last error
    ...
    throw new Error(`Gemini API error from all fallback models: ${lastError?.message || 'Unknown error'}`);
  }
