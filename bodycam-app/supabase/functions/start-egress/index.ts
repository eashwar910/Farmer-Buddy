...
serve(async (req) => {
  ...
  try {
    ...
    adminToken.addGrant({ roomCreate: true, roomList: true, roomAdmin: true, roomRecord: true });
    const adminJwt = await adminToken.toJwt();

    const roomName       = `shift-${shiftId}`;
    const participantId  = user.id;
    const filenamePrefix = `${shiftId}_${participantId}_chunk_`;

    // ── 7. Parse S3 endpoint ─────────────────────────────────────────────────
    const s3EndpointUrl  = new URL(s3Endpoint);
    const s3EndpointHost = s3EndpointUrl.host;
      ...
      segment_outputs: [
        {
          ...
          playlist_name: `${shiftId}_${participantId}_playlist.m3u8`,
          live_playlist_name: '',
          segment_duration: 15,
          protocol: 0,
          output: {
    ...
    const egressRes = await fetch(
      `${livekitApiUrl}/twirp/livekit.Egress/StartRoomCompositeEgress`,
      {
        method: 'POST',
        headers: {
    ...
    if (!egressRes.ok) {
      ...
      return new Response(
        JSON.stringify({ error: `Egress API error ${egressRes.status}: ${errText}` }),
    ...
    return new Response(
      JSON.stringify({ egressId, recordingId: recording?.id ?? null }),
